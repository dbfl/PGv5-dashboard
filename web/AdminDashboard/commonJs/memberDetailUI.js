...
$(document).ready(function() {
	...
	$("#destinationGuid").val(destinationGuid);
	$("#authenticatePhone").click(function() {
		$this.button('loading');
		verifyCarrier();
	});

	$("#verifyPhoneBut").click(function() {
		$this.button('reset');
		$("#verifyPhone").removeClass("btn-primary");
		$("#verifyPhone").addClass("btn-danger");
...
var displayBankList = function(displayBankList) {
	...
	if (displayBankList != undefined && displayBankList != null && displayBankList != '') {
		for (var i = 0; i < displayBankList.length; i++) {
			...
			nameVerify = displayBankList[0].nmVerify;
			getNamePriorityOne = displayBankList[0].name;
			if(displayBankList[0].inquiredName != undefined && displayBankList[0].inquiredName != null && displayBankList[0].inquiredName != ''){
				getEnquireName = displayBankList[0].inquiredName;
			}else{
				getEnquireName = null;
			}
		}
		if (nameVerify != undefined && nameVerify != null && nameVerify != '') {
			if (isAdmin == "false") {
				$(".verifyNameButton").hide();
			} else {
				var verifyNameStatus = nameVerify.verifySt;
				if (verifyNameStatus == "NEED_REVIEW" || verifyNameStatus == "UNVERIFIED" || verifyNameStatus == "UNMATCHED") {
					$("#reviewNameButton").show();
					$("#verifySuccessNameButton").hide();
					$("#bankNameVerifyRow").css("background-color", "#CCCCCC");
					$("#bankNameVerifyRow").css("color", "#003366");
					$(".verifiedActionClass").show();
					if (verifyNameStatus == "UNVERIFIED" || verifyNameStatus == "UNMATCHED") {
						if (verifyNameStatus == "UNVERIFIED") {
							$("#reviewNameButton").text("Verify needed");
						} else {
							$("#reviewNameButton").text("un-match");
							$(".verifiedActionClass").hide();
						}
						$('#reviewNeededOption').show();
						$('#notMatchOption').hide();

					} else if (verifyNameStatus == "NEED_REVIEW") {
						$("#reviewNameButton").text("Review needed");
						$('#reviewNeededOption').hide();
						$('#notMatchOption').show();
					}

				} else {
					$("#reviewNameButton").hide();
					$("#verifySuccessNameButton").show();
					$(".verifiedActionClass").hide();
					$("#bankNameVerifyRow").css("background-color", "#586B7D");
					$("#bankNameVerifyRow").css("color", "#ffffff");
				}
			}

			/**Change status function***/
			$(".verifyNameButton").click(function() {
				$("#seyfertRegisteredName").text(getNamePriorityOne.fullname);
				if(getEnquireName != null){
					$("#bankRegisteredName").text(getEnquireName.fullname);
				}else{
					$("#bankRegisteredName").text("Enquire name not exist");
				}
			});

			/**Change status function***/
			$("#changeBankNameVerifyStatusButton").click(function() {
				var confirmStatus = $("#verifyStatus").val();
				var verifyKey = nameVerify.verifyKey;
				rawData = 'callback=?&reqMemGuid=' + merchantGUID + '&verifySt=' + confirmStatus + '&verifyKey=' + verifyKey;
				param = 'reqMemGuid=' + merchantGUID + '&encReq=' + ENCODE_URI_COMPONENT(ENCRYPT_REQUEST_PARAMETERS(rawData));
				url = TARGET_URI + 'v5a/member/verify?_method=PUT&';

				AJAX_REQUEST.AJAX.CALL_SERVICE(url, method, param, function(data, status) {
					//alert(data.status);
					if (data.status == "SUCCESS") {
						if (data.data.result.verifySt == "VERIFIED") {
							$("#bankNameVerifyRow").css("background-color", "#586B7D");
							$("#bankNameVerifyRow").css("color", "#ffffff");
							$("#reviewNameButton").hide();
							$("#verifySuccessNameButton").show();
							...
							if (data.data.result.verifySt == "NEED_REVIEW") {
								$("#reviewNameButton").text("Review needed");
								$("#reviewNameButton").show();
								$("#verifySuccessNameButton").hide();
								$("#bankNameVerifyRow").css("background-color", "#18A689");
								$("#bankNameVerifyRow").css("color", "#ffffff");

								$('#reviewNeededOption').hide();
								$('#notMatchOption').show();
								$(".verifiedActionClass").show();
							} else if(data.data.result.verifySt == "UNMATCHED") {
								$("#reviewNameButton").text("un-match");
								$(".verifiedActionClass").hide();
							}
						}
						$("#verifyStatus").val("blankVal");
					} else {
					}
				});
			});
...
var myCounter = new Countdown({
	seconds : 179, // number of seconds to count down
	onUpdateStatus : function(sec) {
		$("#counterText").text(sec + " Seconds left..");
		$("#verifyPhone").removeAttr("disabled");
		$this.button('reset');
	},
	onCounterEnd : function() {
		$("#verifyPhone").attr("disabled", "disabled");
		$("#counterText").text('Time out, please try again!');
		$this.button('reset');
	}
});

var verifyCarrier = function() {
	var destinationGuid = $("#destinationGuid").val();
	var carrier = $("#tcarrier").val();

	rawData = 'callback=?&reqMemGuid=' + merchantGUID + '&dstMemGuid=' + destinationGuid + '&infoCd=MEM_INFO_TELECOMPANY&info=' + carrier;
	param = 'reqMemGuid=' + merchantGUID + '&encReq=' + ENCODE_URI_COMPONENT(ENCRYPT_REQUEST_PARAMETERS(rawData));
...
var verifyGender = function() {
	var destinationGuid = $("#destinationGuid").val();
	var gender = $("#gender").val();
	rawData = 'callback=?&reqMemGuid=' + merchantGUID + '&dstMemGuid=' + destinationGuid + '&infoCd=MEM_INFO_GENDER&info=' + gender;
	param = 'reqMemGuid=' + merchantGUID + '&encReq=' + ENCODE_URI_COMPONENT(ENCRYPT_REQUEST_PARAMETERS(rawData));
	url = TARGET_URI + 'v5a/member/createMemInfo?_method=post&';
	method = "POST";
	AJAX_REQUEST.AJAX.CALL_SERVICE(url, method, param, function(data, status) {
...
var verifyIsDomestic = function() {
	var destinationGuid = $("#destinationGuid").val();
	var isDomestic = $("#isDomestic").val();
	rawData = 'callback=?&reqMemGuid=' + merchantGUID + '&dstMemGuid=' + destinationGuid + '&infoCd=MEM_INFO_IS_DOMESTIC&info=' + isDomestic;
	param = 'reqMemGuid=' + merchantGUID + '&encReq=' + ENCODE_URI_COMPONENT(ENCRYPT_REQUEST_PARAMETERS(rawData));
	url = TARGET_URI + 'v5a/member/createMemInfo?_method=post&';
	method = "POST";
	AJAX_REQUEST.AJAX.CALL_SERVICE(url, method, param, function(data, status) {
...
var verifyDob = function() {
	var destinationGuid = $("#destinationGuid").val();
	var dobirth = $("#dateofBirth").val();
	rawData = 'callback=?&reqMemGuid=' + merchantGUID + '&dstMemGuid=' + destinationGuid + '&infoCd=MEM_INFO_BIRTH&info=' + dobirth;
	param = 'reqMemGuid=' + merchantGUID + '&encReq=' + ENCODE_URI_COMPONENT(ENCRYPT_REQUEST_PARAMETERS(rawData));
	url = TARGET_URI + 'v5a/member/createMemInfo?_method=post&';
	method = "POST";
	AJAX_REQUEST.AJAX.CALL_SERVICE(url, method, param, function(data, status) {
...
var verify = function() {
	...
	rawData = 'callback=?&reqMemGuid=' + merchantGUID + '&dstMemGuid=' + destinationGuid;
	param = 'reqMemGuid=' + merchantGUID + '&encReq=' + ENCODE_URI_COMPONENT(ENCRYPT_REQUEST_PARAMETERS(rawData));
	url = TARGET_URI + 'v5/member/util/phoneAuth/request?_method=post&';
	method = "POST";
	AJAX_REQUEST.AJAX.CALL_SERVICE(url, method, param, function(data, status) {
		if (data.status == "ERROR") {
			alert(data.status + ":" + data.data.cdKey);
			$this.button('reset');
		} else {
			$("#sixDigitCode").show();
			myCounter.start();
			...
			$("#verifyPhone").click(function() {
				...
				rawData = 'callback=?&reqMemGuid=' + merchantGUID + '&dstMemGuid=' + destinationGuid + '&code=' + code + '&aid=' + aid;
				param = 'reqMemGuid=' + merchantGUID + '&encReq=' + ENCODE_URI_COMPONENT(ENCRYPT_REQUEST_PARAMETERS(rawData));
				url = TARGET_URI + 'v5/member/util/phoneAuth/verify?_method=post&';
				method = "POST";
				AJAX_REQUEST.AJAX.CALL_SERVICE(url, method, param, function(data, status) {
					if (data.status == "SUCCESS") {
						alert(data.data.status);
						$("#counterText").hide();
						$("#verifyPhone").removeClass("btn-danger");
						$("#verifyPhone").addClass("btn-primary");
